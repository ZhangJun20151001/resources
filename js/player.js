...
// else {
	...
	"open" != geci ? (songFrom33 = "鍏抽棴", hasgeci = !1) : (songFrom33 = "寮€鍚�", hasgeci = !0);
	...
	$songName.html('<a style="color:#f00">鍒濆鍖栧け璐�</a>');
	$songFrom.html('<a style="color:#f00">褰卞瓙</a>');
	$songFrom1.html('<a href="http://www.neice.org" target="_blank" style="color:#f00">NeiCe.ORG</a>');
	$songFrom3.html('<i class="fa fa-times-circle"></i> 姝岃瘝鏈浇鍏�');
	$player.css({
		background: "#38343e"
	});
	$play.css({
	...
	var wenkmMedia = {
		...
		error: function() {
			clearInterval(cicleTime);
			$player.removeClass("playing");
			wenkmTips.show(NeiCeLists [albumId].song_list[songId].song_title + " - 璧勬簮鑾峰彇澶辫触锛屾挱鏀句笅涓€鏇诧紒");
			setTimeout(function() {
				wenkmMedia.prev()
			}, 1E3);
		...
		getInfos: function(a) {
			...
			audio.src = mp3url;
			$songFrom5.show();
			$songFrom5.html('<a class="down"><i class="fa fa-cloud-download" title="涓嬭浇锛�' + NeiCeLists [albumId].song_list[songId].song_title + " - " + NeiCeLists [albumId].song_list[songId].singer + '"></i></a>');
			$(".down").click(function() {
			...
			c.error = function() {
				...
				setTimeout(function() {
					wenkmTips.show(NeiCeLists [albumId].song_list[songId].song_title + " - 涓撹緫鍥剧墖鑾峰彇澶辫触锛�")
				}, 4E3)
			};
			$cover.html(c);
			audio.volume = volume; - 1 != window.document.cookie.indexOf("player=") ? (wenkmMedia.pause(), $("#wenkmLrc").hide(), setTimeout(function() {
				...
				wenkmTips.show("鎾斁鍣ㄨ嚜鍔ㄥ仠姝㈡挱鏀�")
			}, 3E3)) : "close" != auto ? (wenkmTips.show("寮€濮嬫挱鏀� - " + NeiCeLists [albumId].song_list[songId].song_title), audio.play(), $cover.addClass("coverplay")) : (wenkmMedia.pause(), $("#wenkmLrc").hide(), setTimeout(function() {
				$(".switch-ksclrc").hide();
				$(".switch-down").css("right", "35px");
				$(".switch-default").css("right", "65px")
			}, 1), setTimeout(function() {
				wenkmTips.show("鎾斁鍣ㄥ凡鍏抽棴鑷姩鎾斁")
			}, 3E3));
			wenkmLrc.load();
	...
	$(".pause", $player).click(function() {
		...
		wenkmTips.show("鏆傚仠鎾斁 - " + NeiCeLists [albumId].song_list[songId].song_title);
		setTimeout(function() {
			wenkmTips.show("鎾斁鍣ㄤ笅娆¤闂皢鍋滄鎾斁")
		}, 4E3);
		$cover.removeClass("coverplay");
	...
	$(".play", $player).click(function() {
		...
		wenkmTips.show("寮€濮嬫挱鏀� - " + NeiCeLists [albumId].song_list[songId].song_title);
		setTimeout(function() {
			wenkmTips.show("鎾斁鍣ㄤ笅娆¤闂皢鑷姩鎾斁")
		}, 4E3);
	...
	$(".switch-ksclrc").click(function() {
		...
		$("#wenkmLrc").hasClass("hide") ? (ycgeci = !1, hasLrc && $songFrom3.html('<i class="fa fa-times-circle"></i> Lrc姝岃瘝鍏抽棴'), wenkmTips.show("姝岃瘝鏄剧ず宸插叧闂�"), songFrom33 = "鍏抽棴", $songFrom4.html('<i class="fa fa-toggle-off" title="鎵撳紑姝岃瘝"></i>')) : (ycgeci = !0, hasLrc && $songFrom3.html('<i class="fa fa-check-circle"></i> Lrc姝岃瘝寮€鍚�'), wenkmTips.show("寮€鍚瓕璇嶆樉绀�"), songFrom33 = "寮€鍚�", $songFrom4.html('<i class="fa fa-toggle-on" title="鍏抽棴姝岃瘝"></i>'))
	});
	$(".switch-default").click(function() {
		...
		$.ajax({
			...
			dataType: "script",
			success: function() {
				wenkmTips.show(NeiCeLists [albumId].song_album + " - 杞藉叆鎴愬姛!");
				$(".switch-default").hide();
				hasdefault = !1;
		...
		creat: {
			...
			song: function(a, c) {
				...
				$("li", $songList).click(function() {
					...
					$(".myhk_pjax_loading_frame,.myhk_pjax_loading").show();
					albumId = a;
					$(this).hasClass(cur) ? ($(".myhk_pjax_loading_frame,.myhk_pjax_loading").hide(), wenkmTips.show("姝ｅ湪鎾斁 - " + NeiCeLists [a].song_list[d].song_title)) : (RootCookies.SetCookie("player", "no", -1), $(this).addClass(cur).siblings().removeClass(cur), songId = $(this).index(), wenkmMedia.getInfos(songId))
				})
			}
		}
	};
	...
	var hasLrc = !1,
		...
		wenkmLrc = {
			load: function() {
				...
				$(".switch-down").css("right", "35px");
				$(".switch-default").css("right", "65px");
				hasgeci ? $songFrom3.html('<i class="fa fa-check-circle"></i> Lrc姝岃瘝' + songFrom33) : $songFrom3.html('<i class="fa fa-times-circle"></i> Lrc姝岃瘝' + songFrom33);
				...
				$.ajax({
					...
					success: function() {
						0 <= cont.indexOf("[00") ? setTimeout(function() {
							songFrom44 = $("#wenkmLrc").hasClass("hide") ? " - Lrc姝岃瘝宸插叧闂紒" : " - Lrc姝岃瘝鑾峰彇鎴愬姛!";
							wenkmLrc.lrc.format(cont)
						}, 500) : (songFrom44 = " - 鏆傛棤姝岃瘝!", $songFrom3.html('<i class="fa fa-times-circle"></i> 鏆傛棤姝岃瘝'), $(".switch-ksclrc").hide(), $(".switch-down").css("right", "35px"), $(".switch-default").css("right", "65px"))
					}
				})
			},
			lrc: {
...
$(window).scroll(function() {
	var a = $(this).scrollTop(),
		...
		b = $(this).height();
	a + b == c ? hasgeci && ycgeci && ($player.addClass("ksclrc"), $("#wenkmLrc").addClass("hide"), $songFrom3.html('<i class="fa fa-times-circle"></i> 姝岃瘝鏆傛椂闅愯棌'), $songFrom4.html('<i class="fa fa-toggle-off" title="姝岃瘝鏆傛椂闅愯棌"></i>'), hasLrc && wenkmTips.show("Lrc姝岃瘝鑷姩闅愯棌")) : hasgeci && ycgeci && ($player.removeClass("ksclrc"), $("#wenkmLrc").removeClass("hide"), hasLrc && $songFrom3.html('<i class="fa fa-check-circle"></i> Lrc姝岃瘝寮€鍚�'), $songFrom4.html('<i class="fa fa-toggle-on" title="鍏抽棴姝岃瘝"></i>'))
});